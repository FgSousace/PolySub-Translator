...
class TranslationModelSpec:
    ...
    def selection_label(self) -> str:
        return (
            f"{self.rank:02d}. {self.display_name} · "
            f"dokładność {self.accuracy_score}/5 · około {self.size_label}"
        )
...
# The list intentionally tops out at about 5.5 GB.  Scores are practical guidance for
# subtitle work, not a claim that one model wins every language pair or benchmark.
MODEL_CATALOG: tuple[TranslationModelSpec, ...] = (
    _spec(
        ...
        "Bardzo wysoka",
        5,
        "najlepszy ogólny balans jakości i rozmiaru dla wielu języków",
        "CC-BY-NC-4.0",
        batch_cap=4,
    ...
    _spec(
        ...
        "Wysoka",
        5,
        "wielojęzyczne tłumaczenie, gdy liczy się jakość bardziej niż szybkość",
        "CC-BY-NC-4.0",
        batch_cap=4,
        note="Model badawczy do użytku niekomercyjnego.",
    ),
    _spec(
        ...
        "Wysoka",
        4,
        "uniwersalne tłumaczenie około 100 języków",
        "MIT",
        batch_cap=6,
    ),
    _spec(
        ...
        "Dobra",
        4,
        "szybsze wielojęzyczne napisy na komputerach z 8 GB RAM",
        "CC-BY-NC-4.0",
        batch_cap=8,
        note="Model badawczy do użytku niekomercyjnego.",
    ),
    _spec(
        ...
        "Dobra",
        4,
        "tłumaczenie między 50 popularnymi językami",
    ...
    _spec(
        ...
        "Dobra",
        3,
        "lekki model ogólny i zgodność z wcześniejszymi wersjami PolySub",
        "MIT",
        batch_cap=16,
    ...
    _spec(
        ...
        "Dobra",
        3,
        "napisy z angielskiego na jeden z 49 pozostałych języków",
        "Sprawdź kartę modelu",
    ...
    _spec(
        ...
        "Dobra",
        3,
        "napisy z jednego z 49 języków na angielski",
        "Sprawdź kartę modelu",
    ...
    _spec(
        ...
        "Dobra dla tej pary",
        4,
        "angielskie filmy tłumaczone na polski przy małym pobraniu",
        "Apache-2.0",
        sources=frozenset({"en"}),
    ...
    _spec(
        ...
        "Dobra dla tej pary",
        4,
        "polskie napisy tłumaczone na angielski",
        "Apache-2.0",
    ...
    _spec(
        ...
        4,
        2,
        "Bardzo dobra dla tej pary",
        5,
        "niemieckie filmy tłumaczone na polski (Tatoeba BLEU 41,2)",
        "Apache-2.0",
    ...
    _spec(
        ...
        "Bardzo dobra dla tej pary",
        5,
        "hiszpańskie filmy tłumaczone na polski (Tatoeba BLEU 44,6)",
        "Apache-2.0",
    ...
    _spec(
        ...
        "Bardzo dobra dla tej pary",
        5,
        "francuskie filmy tłumaczone na polski (Tatoeba BLEU 40,7)",
        "Apache-2.0",
    ...
    _spec(
        ...
        "Dobra dla tej pary",
        4,
        "ukraińskie dialogi tłumaczone na polski",
        "Apache-2.0",
    ...
    _spec(
        ...
        3,
        "arabskie dialogi tłumaczone na polski",
        "Apache-2.0",
    ...
    _spec(
        ...
        "Podstawowa dla tej pary",
        3,
        "japońskie dialogi tłumaczone na polski",
        "Apache-2.0",
    ...
    _spec(
        ...
        "Bardzo dobra dla tej pary",
        5,
        "polskie napisy tłumaczone na niemiecki (Tatoeba BLEU 47,8)",
        "Apache-2.0",
        sources=frozenset({"pl"}),
        targets=frozenset({"de"}),
        batch_cap=16,
    ),
    _spec(
        18,
        ...
        "Bardzo dobra dla tej pary",
        5,
        "polskie napisy tłumaczone na hiszpański (Tatoeba BLEU 46,9)",
        "Apache-2.0",
        sources=frozenset({"pl"}),
        targets=frozenset({"es"}),
        batch_cap=16,
    ),
    _spec(
        19,
        ...
        "Bardzo dobra dla tej pary",
        5,
        "litewskie dialogi tłumaczone na polski (Tatoeba BLEU 53,6)",
        "Apache-2.0",
        sources=frozenset({"lt"}),
        targets=frozenset({"pl"}),
        batch_cap=16,
    ),
    _spec(
        ...
        "Podstawowa dla tej pary",
        2,
        "norweskie napisy na polski; lekki, ale wyraźnie mniej dokładny (BLEU 20,9)",
        "Apache-2.0",
        sources=frozenset({"no"}),
        targets=frozenset({"pl"}),
        batch_cap=16,
    ),
)
...
# Kept only so old checkpoints/configuration and the MADLAD engine path remain
# readable after the oversized model was removed from the downloadable catalog.
LEGACY_MODEL_SPECS: tuple[TranslationModelSpec, ...] = (
    _spec(
        0,
        "madlad400-3b",
        "google/madlad400-3b-mt",
        "MADLAD-400 3B (legacy)",
        ModelFamily.MADLAD,
        11.9,
        24,
        16,
        "Bardzo wysoka",
        5,
        "zgodność ze starszymi ustawieniami; nie jest już oferowany do pobrania",
        "Apache-2.0",
        batch_cap=2,
        note="Usunięty z menedżera ze względu na rozmiar.",
    ),
)
MODEL_BY_ID = {model.id: model for model in (*MODEL_CATALOG, *LEGACY_MODEL_SPECS)}
